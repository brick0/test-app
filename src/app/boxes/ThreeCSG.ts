...
export class ThreeBSP {
  ...
  constructor(geometry) {
    ...
    for (i = 0, _length_i = geometry.faces.length; i < _length_i; i++) {
      ...
      polygon = new Polygon(null,null,null);
  ...
  splitPolygon = function (polygon, coplanar_front, coplanar_back, front, back) {
    ...
    if (classification === COPLANAR) {
      ...
      if (f.length >= 3) front.push(new Polygon(f,null,null).calculateProperties());
      if (b.length >= 3) back.push(new Polygon(b,null,null).calculateProperties());
    }
  };
};
